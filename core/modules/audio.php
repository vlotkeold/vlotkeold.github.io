	...
	switch($act){
		...
		case "send":
			...
			if($format == 'mp3' AND isset($lnk) AND !empty($lnk) AND $config['audio_mod_add'] == 'yes'){
				...
				if(strpos($check_url[0], '200')){
					...
					$db->query("INSERT INTO `".PREFIX."_audio` SET auser_id = '".$user_id."', url = '".$lnk."', artist = '".$artist."', name = '".$name."',  full_name = '".$artist." - ".$name."', adate = '".$server_time."'");
		...
		case "editsave":
			...
			if($check['auser_id'] == $user_id){
				$db->query("UPDATE `".PREFIX."_audio` SET artist = '{$artist}', name = '{$name}', full_name = '{$artist} - {$name}' WHERE aid = '".$aid."'");
		...
		case "addmylist":
			...
			if($check){
				$db->query("INSERT INTO `".PREFIX."_audio` SET auser_id = '".$user_id."', url = '".$db->safesql($check['url'])."', artist = '".$db->safesql($check['artist'])."', name = '".$db->safesql($check['name'])."', full_name = '".$db->safesql($check['artist'])." - ".$db->safesql($check['name'])."', adate = '".$server_time."', stype = 1");
				$db->query("UPDATE `".PREFIX."_users` SET user_audio = user_audio+1 WHERE user_id = '".$user_id."'");
				mozg_mass_clear_cache_file('user_'.$user_id.'/audios_profile|user_'.$user_id.'/profile_'.$user_id);
			}
			die();
		...
		case "upload":
			...
			if($type == 'mp3' AND $config['audio_mod_add'] == 'yes' AND $file_size < 50000000){
