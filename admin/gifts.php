	...
	if($price){
		if(in_array(strtolower($type), $allowed_files) AND in_array(strtolower($type_2), $allowed_files)){
			...
			if($image_size < 200000){
				...
				msgbox('Error', 'Укажите категорию(1,2,4)', 'javascript:history.go(-1)');
		} else
...
Цена: <input type="text" id="price{$row['gid']}" class="inpu" value="{$row['price']}" />
Категория: <input type="text" id="category{$row['gid']}" class="inpu" value="{$row['category']}" /><br />
[ <a href="?mod=gifts" onClick="window.location.href='?mod=gifts&act=edit&id={$row['gid']}&price='+document.getElementById('price{$row['gid']}').value; return false">изменить цену</a> ] [ <a href="?mod=gifts" onClick="window.location.href='?mod=gifts&act=edit&id={$row['gid']}&category='+document.getElementById('category{$row['gid']}').value; return false">изменить категорию</a> ] [ <a href="?mod=gifts&act=del&id={$row['gid']}">удалить</a> ]
</div>
HTML;
}
...
<div class="fllogall" style="width:180px">Категория (1 - Дружба, 2 - День Рождения, 4 - Романтика):</div>
