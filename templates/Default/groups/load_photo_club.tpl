...
$(document).ready(function(){
	Xajax = new AjaxUpload('upload', {
		action: '/index.php?go=clubs&act=loadphoto&id={id}',
		name: 'uploadfile',
		...
		onComplete: function (file, data){
			...
			if(data == 'big_size'){
				...
				Box.Close(0, 1);
				$('#ava').attr('src', '/uploads/clubs/{id}/'+data);
				$('#del_pho_but').show();
			}
		}
	});
...
<div class="load_photo_quote">Вы можете загрузить сюда только фотографию сообщества. Поддерживаются форматы JPG, PNG и GIF.</div>
