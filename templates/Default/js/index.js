...
function handlePageParams(params) {
  ...
  var lnks = ['friends', 'albums' + vk.id, 'video', '', 'notes', 'groups', 'events', 'gifts.php?act=wishlist', 'apps', 'ads?act=office', 'feed' + (ge('l_nwsf') ? '?section=notifications' : ''), 'pages', 'docs'];
...
function lockButton(el) {
  if (!(el = ge(el))) return;
  var btn = (el.tagName.toLowerCase() == 'button'), d = btn ? 0 : ((browser.msie6 || browser.msie7) ? 2 : 4), tEl = btn ? el : geByClass1('file_button_text', el);
