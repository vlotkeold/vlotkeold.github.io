...
var apps = {
	...
	loadSettings: function(i,hash) {
 	...
 	saveSettings: function(aid, hash, user_id){
 		var i = $('#app_pay_add').val();
 		$.post('/index.php?go=apps&act=save_settings',{aid: aid, add: i, hash: hash, user_id: user_id}, function(data){	
			...
			$.post('/index.php?go=editapp&act=checkAdmin', {id: upage}, function(d){
				...
				if(d[0]){
					if(d[1])
						...
						$('#feedimg').attr('src', '/templates/Default/images/no_ava_50.png');
...
var Photo = {
	...
	wall_add_like: function(rec_id, user_id){
		...
		$('#wall_like_cnt'+rec_id).html(wall_like_cnt).css('color', '#2f5879');
		$('#wall_active_ic'+rec_id).addClass('public_wall_like_yes');
		$('#wall_like_link'+rec_id).attr('onClick', 'Photo.wall_remove_like('+rec_id+', '+user_id+')');
		$('#like_user'+user_id+'_'+rec_id).show();
		updateNum('#like_text_num'+rec_id, 1);
		
		$.post('/index.php?go=albums&act=wall_like_yes', {rec_id: rec_id});
	},
	wall_remove_like: function(rec_id, user_id){
		var wall_like_cnt = parseInt($('#wall_like_cnt'+rec_id).text())-1;
		if(wall_like_cnt <= 0){
		...
		$('#wall_like_cnt'+rec_id).html(wall_like_cnt).css('color', '#95adc0');
		$('#wall_active_ic'+rec_id).removeClass('public_wall_like_yes');
		$('#wall_like_link'+rec_id).attr('onClick', 'Photo.wall_add_like('+rec_id+', '+user_id+')');
		$('#Xlike_user'+user_id+'_'+rec_id).hide();
		$('#like_user'+user_id+'_'+rec_id).hide();
		updateNum('#like_text_num'+rec_id);

		$.post('/index.php?go=albums&act=wall_like_remove', {rec_id: rec_id});
	},
	wall_like_users_five: function(rec_id){
		$('.public_likes_user_block').hide();
		if(!ge('like_cache_block'+rec_id) && $('#wall_like_cnt'+rec_id).text() && $('#update_like'+rec_id).val() == 0){
			$.post('/index.php?go=albums&act=wall_like_users_five', {rec_id: rec_id}, function(data){
				$('#likes_users'+rec_id).html(data+'<span id="like_cache_block'+rec_id+'"></span>');
				$('#public_likes_user_block'+rec_id).show();
			});
		} else
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=albums&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	}
}
...
var friends = {
	...
	del: function(del_user_id){
		...
		$.post('/index.php?go=friends&act=del', {del_user_id: del_user_id}, function(){
			$('#addsubscription_load').hide();
			$('#text_add_subscription').text(lang_subscription2);
			$('#lnk_unsubscription').attr('onClick', 'friends.sending_demand('+del_user_id+'); return false');
		});
	},
...
fall: function(uid, page_num, c){ 
  ...
  if(page_num) 
    ...
    page_num = 1; 
		...
		Box.Page('/index.php?go=subscriptions&act=fall', 'uid='+uid+page, 'subscriptions_'+page_num, 625, 'Подписчики', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, cache);
	...
	go: function(){
		...
		//if(query != 0 && query != 'Поиск' && query != 'Начните вводить любое слово или имя'){
			if(CheckRequestSearch(location.href) && type == 1){
				...
				if(miro != 0) all_queryeis_miro = '&miro='+miro;
				else all_queryeis_miro = '';
...
if(jizn != 0) all_queryeis_jizn = '&jizn='+jizn;
				else all_queryeis_jizn = '';
if(ludi != 0) all_queryeis_ludi = '&ludi='+ludi;
				else all_queryeis_ludi= '';
if(kurenie != 0) all_queryeis_kurenie = '&kurenie='+kurenie;
				else all_queryeis_kurenie = '';
if(alkogol != 0) all_queryeis_alkogol = '&alkogol='+alkogol;
				else all_queryeis_alkogol = '';

				res_sort_query = all_queryeis_sex+all_queryeis_day+all_queryeis_month+all_queryeis_year+all_queryeis_country+all_queryeis_city+all_queryeis_online+all_queryeis_user_photo+all_queryeis_sp+all_queryeis_miro+all_queryeis_mesto+all_queryeis_dolj+all_queryeis_chast+all_queryeis_countrysl+all_queryeis_nacalosl+all_queryeis_jizn+all_queryeis_ludi+all_queryeis_kurenie+all_queryeis_alkogol;
			} else
				res_sort_query = '';
...
var wall = {
	...
	event: function(event){
		...
		if(oi != 'wall_tab' && oi != 'wall_input' && oi != 'wall_textarea' && oi != 'podpis' && oi != 'wall_text' && oi != 'wall_send' && oi != 'wall_attach' && oi != 'wall_attach_link' && !attach_files)
			wall.form_close();

		if(fast_oi != 'fast_form' && fast_oi != 'fast_link' && fast_oi != 'fast_inpt' && fast_oi != 'fast_text' && fast_oi != 'fast_buts' && for_group != 'for_group')
	...
	attach_insert: function(type, data, action_url, uid){
		...
		//Если вставляем стикер
		if(type == 'stiker'){
			Box.Close('attach_stiker', 1);
			stiker = data.split('stiker/');
			res_attach_id = 'smile_'+attach_id;
			$('#attach_files').append('<span id="attach_file_'+res_attach_id+'" class="attach_file"><img src="'+data+'" class="wall_attach_stiker fl_l" onClick="wall.attach_delete(\''+res_attach_id+'\', \'stiker|'+stiker[1]+'||\')" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_stiker_\')" onMouseOut="myhtml.title_close(\''+res_attach_id+'\')" id="wall_stiker_'+res_attach_id+'" style="margin-top:0px" /></span>');
			$('#vaLattach_files').val($('#vaLattach_files').val()+'stiker|'+stiker[1]+'||');
		}
		
		//Если вставляем стикер
		if(type == 'stiker_cat'){
			Box.Close('attach_stiker_cat', 1);
			stiker_cat = data.split('stiker_cat/');
			res_attach_id = 'smile_'+attach_id;
			$('#attach_files').append('<span id="attach_file_'+res_attach_id+'" class="attach_file"><img src="'+data+'" class="wall_attach_stiker fl_l" onClick="wall.attach_delete(\''+res_attach_id+'\', \'stiker_cat|'+stiker_cat[1]+'||\')" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_stiker_cat_\')" onMouseOut="myhtml.title_close(\''+res_attach_id+'\')" id="wall_stiker_cat_'+res_attach_id+'" style="margin-top:0px" /></span>');
			$('#vaLattach_files').val($('#vaLattach_files').val()+'stiker_cat|'+stiker_cat[1]+'||');
		}
	...
	attach_addstiker: function(){
		viiBox.clos('boxbuy', 1)
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_stiker', 290, lang_wall_atttach_addstiker, lang_wall_attach_stiker, lang_box_canсel, '', '', 0, 1, 1, 1);
	},
	attach_addstiker_cat: function(){
		viiBox.clos('boxbuy', 1)
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_stiker_cat', 290, lang_wall_atttach_addstiker, lang_wall_attach_stiker_cat, lang_box_canсel, '', '', 0, 1, 1, 1);
	},
	attach_addphoto: function(id, page_num, notes){
		...
		Box.Page('/index.php?go=albums&act=all_photos_box', page+notes, 'all_photos_'+page_num, 625, 'Прикрепление фотографии', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
...
var gStatus = {
	...
	startTell: function(){
		...
		if(tell_friends){
			if(text != 0){
				$.post('/index.php?go=wall&act=sendstatus', {wall_text: wall_text, for_user_id: for_user_id[1]}, function(data){
					$('#wall_records').html(data);
					$('#wall_all_record').html('');
...
var stikers = {
	buycat_1: function() {
		$.post('/index.php?go=stikers&act=add_category_1_buy', function(data){
			if(data == 'now_cat_stik_1') {
				Box.Info('err', 'Ошибка', 'У Вас уже есть набор "Персик"!', 250, 1500);
			} else if(data == 'n_money') {
				Box.Info('err', 'Ошибка', 'У Вас недостаточно голосов!', 250, 1500);
			} else {
				$('#cat_stik_1ok').addClass('button_div_gray');
				Box.Info('err', 'Успешно', 'Вы купили набор "Персик"!', 250, 1500);
			}
		});
	},
	boxbuy: function(user_id, c, j){
		if(!j) j = 0;
		if(!j) viiBox.start();
		$.post('/index.php?go=stikers&act=boxbuy', {user_id: user_id, section: j}, function(d){
			if(!j) viiBox.win('boxbuy', d);
			else {
				var type = $('#type').val();
				$('#tb_box_'+type).hide();
				$('#tb_box_'+j).show().html('<div class="gifts_section clear_fix" id="boxbuy">'+d+'</div>');
				$('#type').val(j);
			}
		})
	},
	open_free_cat1: function(user_id, c, j){
		viiBox.clos('boxbuy', 1);
		if(!j) j = 0;		
		$.post('/index.php?go=stikers&act=open_free_cat1', {user_id: user_id, section: j}, function(d){
			if(!j) viiBox.win('open_free_cat1', d);
			else {
				var type = $('#type').val();
				$('#tb_box_'+type).hide();
				$('#tb_box_'+j).show().html('<div class="gifts_section clear_fix" id="open_free_cat1">'+d+'</div>');
				$('#selection_'+type).removeClass('summary_tab_sel').addClass('summary_tab');
				$('#selection_'+j).addClass('summary_tab_sel').removeClass('summary_tab');
				$('#type').val(j);
			}
	...
	showstikers: function(id){
		$('#s'+id).show();
	},
	showhide: function(id){
		$('#s'+id).hide();
	},
	send: function(sfid, fid){
		viiBox.clos('open_free_cat1', 1);
		var privacy = $('#private_stikers'+sfid).val();
		var msgfstiker = $('#msgfstiker'+sfid).val();
		$('#box_loading').show().css('margin-top', '-5px');
		$.post('/index.php?go=stikers&act=send', {for_user_id: fid, stiker: sfid, privacy: privacy, msg: msgfstiker}, function(d){
			if(d == 1){
				addAllErr(lang_stikers_tnoubm, 3000);
				Box.Close();
			} else {
				Box.Close();
				Box.Info('stikerok', lang_stikers_oktitle, lang_stikers_oktext, 250, 2000);
			}
		});
	},
	showStikersPrice: function(j,text){
		$('#stikers_'+j).append('<div id="gift_price"><span id="stikers_price_str">'+text+'</span><b class="rbottom"><b class="r"></b></b></div>');
	},
	hideStikersPrice: function(){
		$('#stikers_price').detach();
	}
}
...
var clubs = {
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://ivinete.ru/id', '');
	...
	send_new_admin: function(id, new_admin_id){
		...
		var data = '<div class="public_oneadmin" id="admin'+new_admin_id+'"><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false"><img src="'+ava+'" align="left" width="32" /></a><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false">'+adm_name+'</a><br /><a href="/" onClick="clubs.deladmin(\''+id+'\', \''+new_admin_id+'\'); return false"><small>Удалить</small></a></div>';
	...
	wall_attach_addphoto: function(id, page_num, public_id){
		...
		Box.Page('/index.php?go=clubs&act=photos', 'public_id='+public_id+page, 'c_all_photos_'+page_num, 625, 'Прикрепление фотографии', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на яВинете', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на яВинете.."  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'groups.wall_video_add_select()');
	...
	subscribes_groups: function(j,i){
		...
		$.post('/index.php?go=clubs&act=subscribes'+z, {pid: j}, function(data){
	...
	page_subscribes_groups: function(j,i,f){
		...
		if((parseInt($('#subs_count').text()) > parseInt(page_cnt)*40 || $('#fans_rowsmembers').size() < 40) && $('#loading').val() == 0) {
			...
			$('#loading').val(1);
			if(!f) f = 0;
			$.post('/index.php?go=clubs&act=subscribes'+z, {page_cnt: page_cnt, pid: j, fave: f}, function(data){
				var d = data.split('<f>');
	...
	friends_groups: function(j,h){
		...
		if(j == 1) {
			...
			$('#offset').val('1');
			$('#type').val('1');
			clubs.page_subscribes_groups(h,'1',1);
		} else {
			...
			$('#tb_title').html('Друзья в группе');
			$('#offset').val('1');
			$('#type').val('2');
			clubs.page_subscribes_groups(h,'2',1);
		}
		
	},
	miniature: function(j){
		Page.Loading('start');
		$.post('/index.php?go=clubs&act=miniature&cid='+j, function(d){
			Page.Loading('stop');
			if(d == 1) 
	...
	vPhoto2: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=clubs&act=vphoto_wall', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var groups = {
	...
	links: function(id){
		Box.Page('/index.php?go=groups&act=links', 'id='+id, 'links', 450, 'Ссылки', 'Закрыть', 0, 0, 300, 1, 1, 1, 0, 0);
	},
	delfeedback: function(id, uid){
		$('#f'+uid+', #fb'+uid).remove();
		var si = $('.public_obefeed').size();
	...
	wall_send: function(id){
		...
		if(wall_text != 0 || attach_files != 0){
			butloading('wall_send', 56, 'disabled');
			$.post('/index.php?go=groups&act=wall_send', {id: id, podpis: podpis, wall_text: wall_text, attach_files: attach_files, vote_title: $('#vote_title').val(), vote_answer_1: $('#vote_answer_1').val(), vote_answer_2: $('#vote_answer_2').val(), vote_answer_3: $('#vote_answer_3').val(), vote_answer_4: $('#vote_answer_4').val(), vote_answer_5: $('#vote_answer_5').val(), vote_answer_6: $('#vote_answer_6').val(), vote_answer_7: $('#vote_answer_7').val(), vote_answer_8: $('#vote_answer_8').val(), vote_answer_9: $('#vote_answer_9').val(), vote_answer_10: $('#vote_answer_10').val()}, function(data){
				if($('#rec_num').text() == 'Нет записей')
					$('.albtitle').html('<b id="rec_num">1</b> запись');
				else
					updateNum('#rec_num', 1);
	...
	wall_send_comm: function(rec_id, public_id){
		var wall_text = $('#fast_text_'+rec_id).val();
		var for_groups = $('#for_group'+rec_id).val();
		var answer_comm = $('#answer_comment_'+rec_id).val();

		if(wall_text != 0){
			butloading('fast_buts_'+rec_id, 56, 'disabled');
			$.post('/index.php?go=groups&act=wall_send_comm', {rec_id: rec_id, wall_text: wall_text, public_id: public_id, for_groups: for_groups, answer_comm: answer_comm}, function(data){
				$('#fast_form_'+rec_id+', #fast_comm_link_'+rec_id).remove();
				$('#wall_fast_block_'+rec_id).html(data);
				var pattern = new RegExp(/news/i);
				if(pattern.test(location.href)) $('#fast_text_'+rec_id+', #fast_inpt_'+rec_id).css('width', '688px');
	...
	comm_wall_answer: function(rec_id,record_id){
		var j = $('#wall_fast_comment_'+rec_id).find('#name_comment > a');
		$('#reply_to_title-'+record_id).html('<a href="'+j.attr('href')+'">'+j.text()+'</a>');
		$('#answer_comment_'+record_id).val(rec_id);
		wall.fast_open_textarea(record_id, '2');
		$('#fast_text_'+record_id).val(j.text()+', ');
	},
	comm_wall_gotocomment: function(post_id,rec_id,p_id){
		if(!$('#wall_fast_comment_'+post_id).length) {
			groups.wall_all_comments(rec_id, p_id);
			setTimeout(function(){groups.comm_wall_gotocomment_okay(post_id);},200);
		} else groups.comm_wall_gotocomment_okay(post_id);
	},
	comm_wall_gotocomment_okay: function(post_id){
		$('body').animate({scrollLeft: 0, scrollTop:$('#wall_fast_comment_'+post_id).offset().top}, 1000);
		$('#wall_fast_comment_'+post_id).animate({'background-color': '#ECEFF3'});
		setTimeout(function(){$('#wall_fast_comment_'+post_id).animate({'background-color': '#fff'});}, 1500);
	},
	wall_all_comments: function(rec_id, public_id){
		textLoad('wall_all_comm_but_'+rec_id);
		$('#wall_all_but_link_'+rec_id).attr('onClick', '');
	...
	wall_attach_addphoto: function(id, page_num, public_id){
		...
		Box.Page('/index.php?go=groups&act=photos', 'public_id='+public_id+page, 'c_all_photos_'+page_num, 625, 'Прикрепление фотографии', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	wall_attach_insert: function(type, data, action_url){
		if(!$('#wall_text').val())
			wall.form_open();
...
var audio = {
	...
	addBoxComp: function(){
		Box.Close();
		Box.Show('addaudio_comp', 510, lang_audio_add, '<div class="videos_pad"><div class="videos_text">Ограничения<div class="clear"></div><ul class="listing2"><li style="font-weight:normal;color:#000;font-size:11px;margin-top:10px"><span>Аудиофайл не должен превышать 50 Мб и должен быть в формате MP3.</span></li><li style="font-weight:normal;color:#000;font-size:11px;margin-bottom:15px"><span>Аудиофайл не должен нарушать авторские права.</span></li></ul><div class="button_blue fl_l" style="margin-left:170px"><button id="upload">Выбрать файл</button></div><div class="clear"></div></div></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
		$('#audio_lnk').focus();
		$('#box_but').hide();
...
var epage = {
	saveInfo: function(id){
		...
		var videos = $('#videos').val();
		var links = $('#links').val();
		var lastnews = $('#lastnews').val();
		var albums = $('#albums').val();
		var day = $('#day').val();
		var month = $('#month').val();
		var years = $('#years').val();
		var category = $('#public_category').val();
		var pcategory = $('#public_pcategory').val();
		
		if(title != '') {
			if(adres != '') {
				$.post('/index.php?go=editpage&act=saveGeneralInfo', {id:id, title: title, adres: adres, descr: descr, website: website, comments: comments, audio: audio, contact: contact, videos: videos, links: links, lastnews: lastnews, albums: albums, day: day, month: month, years: years, category: category, pcategory: pcategory}, function(data){
	...
	saveNews: function(id){
		var lastnews = $('#lastnews').val();	
				$.post('/index.php?go=editpage&act=saveNews', {id:id, lastnews: lastnews}, function(data){
					$('#info_save').html(lang_infosavepublic);
					$('#info_save').show();
				}); 
	},
	...
	vPhotoG: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=editpage&act=vphoto', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
	...
	vPhoto2: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=editpage&act=vphoto_wall', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var eclub = {
	saveInfo: function(id){
		...
		if(title != '') {
			if(adres != '') {
				$.post('/index.php?go=editclub&act=saveGeneralInfo', {id:id, title: title, adres: adres, descr: descr, comments: comments, contact: contact, val_wall1: val_wall1, val_board: val_board, val_intog: val_intog}, function(data){
					$('#info_save').html(lang_infosaveclub);
					$('#info_save').show();
				}); 
			} else setErrorInputMsg('adres');
		} else setErrorInputMsg('title');
	...
	users_page: function(id){
		...
		if(length < $('#count').val()) {
			...
			$.post('/eclub?act=users&cid='+id+type, {page_cnt: page_cnt}, function(data){
				$('#all_users').html(data);
				$('#page_cnt').val(parseInt($('#page_cnt').val())+1);
			});
		}
	...
	editadmin: function(id,type,typetodelete){
		...
		if(in_array(type, atype)) {
			Page.Loading('start');
			$.post('/index.php?go=editclub&act=editadmin&cid='+pid, {id: id, type: type}, function(data){
				Page.Loading('stop');
				if(type == 'deleteadmin') {
					title = 'Удаление руководителя';
					button_ok = 'Разжаловать руководителя';
					function_ok = 'eclub.deleteadmin('+id+','+f+typetodelete+f+')';
				} else if(type == 'editadmin') {
					title = 'Редактирование руководителя';
					button_ok = 'Сохранить';
					function_ok = 'eclub.saveadmin('+id+','+f+'saveadmin'+f+')';
				} else if(type == 'newadmin') {
					title = 'Назначение руководителя';
					button_ok = 'Назначить руководителем';
					function_ok = 'eclub.saveadmin('+id+','+f+'newadmin'+f+')';
				}
				Box.Show('editadmin', 410, title, data, lang_box_canсel, button_ok, function_ok,'', 0, 0, 0, 0);
			});
		}
	...
	saveadmin: function(id,type){
		...
		var email = $('#gedit_admbox_email').val();
		var phone = $('#gedit_admbox_phone').val();
		$.post('/index.php?go=editclub&act=saveadmin&cid='+pid, {id: id, level: level, blockf: blockf, position: position, email: email, phone: phone, type: type}, function(data){
			Box.Close('editadmin');
			$('#gedit_user_level'+id).html(convertLevel(parseInt(level)));
			if(type == 'newadmin') {
				$('#gedit_user_level'+id).show();
	...
	deleteadmin: function(id,type){
		...
		var email = $('#gedit_admbox_email').val();
		var phone = $('#gedit_admbox_phone').val();
		$.post('/index.php?go=editclub&act=deleteadmin&cid='+pid, {id: id, blockf: blockf, position: position, email: email, phone: phone}, function(data){
			Box.Close('editadmin');
			$('#gedit_user_level'+id).detach();
			if(type != 'onfalldelete') Box.Info('msg_info', '<b>Изменения сохранены.</b>', '<span style="font-size:12px;"><b><a href="'+$('#gedit_user_name'+id).attr('href')+'">'+$('#gedit_user_name'+id).html()+'</a></b> убран из списка руководителей.</span>', 380, 2000);
			if(type == 'onfalldelete') eclub.delusers(id,pid);
		});
	},
	delusers: function(id,cid) {
		$.post('/index.php?go=editclub&act=deleteusers&cid='+pid, {id: id}, function(data){
			$('#gedit_user_action'+id).hide();
			$('#gedit_user_action'+id).html('Удалить из сообщества');
			$('#gedit_users'+id).hide();
			$('#gedit_user_actions'+id).show();
	...
	rebornusers: function(id) {
		var pid = $('#cid').val();
		textLoad('gedit_user_actions'+id);
		$.post('/index.php?go=editclub&act=rebornusers&cid='+pid, {id: id}, function(data){
			$('#gedit_users'+id).show();
			$('#gedit_user_action'+id).show();
			$('#gedit_user_actions'+id).html('Восстановить');
			$('#gedit_user_actions'+id).hide();
	...
	deleteusers: function(id){ 
		...
		textLoad('gedit_user_action'+id);
		if($('#gedit_user_level'+id).html() != '') {
			eclub.editadmin(id,'deleteadmin','onfalldelete');
		} else eclub.delusers(id,pid);
	},
	changePhotos: function(){
		var take = parseInt($('#imgs_id').val());
		if(take+1>=img_logo.length) $('#imgs_id').val(0);
	...
	miniature: function(j){
		Page.Loading('start');
		$.post('/index.php?go=editclub&act=miniature&cid='+j, function(d){
			Page.Loading('stop');
			if(d == 1) 
				addAllErr('Вы пока что не загрузили фотографию.');
			else {
			...
			$(window).keydown(function(event){
				if(event.keyCode == 27) eclub.miniatureClose();
			});
		});
	},
	preview: function(img, selection){
	...
	miniatureSave: function(j){
		...
		var i_height = $('#mi_height').val();
		butloading('miniatureSave', '111', 'disabled', '');
		$.post('/index.php?go=editclub&act=miniature_save&cid='+j, {i_left: i_left, i_top: i_top, i_width: i_width, i_height: i_height}, function(d){
			if(d == 'err') addAllErr('Ошибка');
			else window.location.href = '/'+d;
			butloading('miniatureSave', '111', 'enabled', 'Сохранить изменения');
		});
	...
	vPhoto: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=editclub&act=vphoto', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var about = {
	createbox: function(){
		Box.Show('create', 490, lang_groups_new, '<div style="padding:20px"><div class="videos_text">Название</div><input type="text" class="videos_input" id="title" maxlength="65" /></div>', lang_box_canсel, lang_groups_cretate, 'about.creat()', 100, 0, 0, 0, 0, 'title');
		$('#title').focus();
	},
	creat: function(){
		var title = $('#title').val();
		if(title != 0){
			$('#box_loading').show();
			ge('box_butt_create').disabled = true;
			$.post('/index.php?go=abouts&act=send', {title: title}, function(id){
				Box.Close();
				Page.Go('/about');
			});
		} else
	...
	loadphoto: function(id){
		Box.Page('/index.php?go=abouts&act=loadphoto_page', 'id='+id, 'loadphoto', 400, lang_title_load_photo, lang_box_canсel, 0, 0, 0, 0, 0, 0, 0, 1);
	},
	delphoto: function(id){
		Box.Show('del_photo', 400, lang_title_del_photo, '<div style="padding:15px;">'+lang_del_photo+'</div>', lang_box_canсel, lang_box_yes, 'about.startdelete('+id+')');
	},
	startdelete: function(id){
		$('#box_loading').show();
		ge('box_butt_create').disabled = true;
		$.post('/index.php?go=abouts&act=delphoto', {id: id}, function(){
			$('#ava').attr('src', template_dir+'/images/no_ava.gif');
			$('#del_pho_but').hide();
			Box.Close();
		});
	...
	addcontact: function(id){
		Box.Page('/index.php?go=abouts&act=addfeedback_pg', 'id='+id, 'addfeedback', 400, 'Добавление контактного лица', lang_box_canсel, 'Сохранить', 'about.savefeedback('+id+')', 0, 0, 0, 0, 'upage', 0);
	},
	savefeedback: function(id){
		var upage = $('#upage').val();
		var office = $('#office').val();
		...
		if($('#feedimg').attr('src') != template_dir+'/images/contact_info.png'){
			$('#box_loading').show();
			ge('box_butt_create').disabled = true;
			$.post('/index.php?go=abouts&act=addfeedback_db', {id: id, upage: upage, office: office, phone: phone, email: email}, function(d){
				if(d == 1){
					Box.Info('err', 'Информация', 'Этот пользователь уже есть в списке контактов.', 300, 2000);
					ge('box_butt_create').disabled = false;
					$('#box_loading').hide();
	...
	allfeedbacklist: function(id){
		Box.Page('/index.php?go=abouts&act=allfeedbacklist', 'id='+id, 'allfeedbacklist', 450, 'Контакты', 'Закрыть', 0, 0, 300, 1, 1, 1, 0, 0);
	},
	delfeedback: function(id, uid){
		$('#f'+uid+', #fb'+uid).remove();
		var si = $('.public_obefeed').size();
		updateNum('#fnumu');
		if(si <= 0){
			$('#feddbackusers').html('<div class="line_height color777" align="center">Страницы представителей, номера телефонов, e-mail<br /><a href="/about" onClick="about.addcontact('+id+'); return false">Добавить контакты</a></div>');
			$('.box_conetnt').html('<div align="center" style="padding-top:10px;color:#777;font-size:13px;">Список контактов пуст.</div><style>#box_bottom_left_text{padding-top:6px}</style>');
		}
		$.post('/index.php?go=abouts&act=delfeedback', {id: id, uid: uid});
	},
	editfeedback: function(uid){
		$('#close_editf'+uid).hide();
		$('#editf'+uid).show();
	...
	editfeeddave: function(id, uid){
		...
		$.post('/index.php?go=abouts&act=editfeeddave', {id: id, uid: uid, office: office, phone: phone, email: email});
	},
	checkFeedUser: function(){
		var upage = $('#upage').val();
		var pattern = new RegExp(/^[0-9]+$/);
		if(pattern.test(upage)){
			$.post('/index.php?go=abouts&act=checkFeedUser', {id: upage}, function(d){
				d = d.split('|');
				if(d[0]){
					if(d[1])
						$('#feedimg').attr('src', '/uploads/users/'+upage+'/100_'+d[1]);
	...
	saveinfo: function(id){
		...
		if(!adres_page)	var adres_page = 'about';
		var pattern = new RegExp(/^[a-zA-Z0-9_-]+$/);
		if(pattern.test(adres_page)){
			butloading('pubInfoSave', 55, 'disabled');
			$.post('/index.php?go=about&act=saveinfo', {id: id, title: title, descr: descr, comments: comments, audio: audio, contact: contact, videos: videos, gtype: $('#gtype').val(), web: $('#web').val()}, function(d){
				if(d == 'err_adres')
					Box.Info('err', 'Ошибка', 'Такой адрес уже занят', 130, 1500);
				else
					if($('#prev_adres_page').val() == adres_page)
	...
	wall_send: function(id){
		...
		if(wall_text != 0 || attach_files != 0){
			butloading('wall_send', 56, 'disabled');
			$.post('/index.php?go=about&act=wall_send', {id: id, wall_text: wall_text, attach_files: attach_files, vote_title: $('#vote_title').val(), vote_answer_1: $('#vote_answer_1').val(), vote_answer_2: $('#vote_answer_2').val(), vote_answer_3: $('#vote_answer_3').val(), vote_answer_4: $('#vote_answer_4').val(), vote_answer_5: $('#vote_answer_5').val(), vote_answer_6: $('#vote_answer_6').val(), vote_answer_7: $('#vote_answer_7').val(), vote_answer_8: $('#vote_answer_8').val(), vote_answer_9: $('#vote_answer_9').val(), vote_answer_10: $('#vote_answer_10').val()}, function(data){
				if($('#rec_num').text() == 'Нет записей')
					$('.albtitle').html('<b id="rec_num">1</b> запись');
				else
					updateNum('#rec_num', 1);
	...
	wall_send_comm: function(rec_id, public_id){
		...
		if(wall_text != 0){
			butloading('fast_buts_'+rec_id, 56, 'disabled');
			$.post('/index.php?go=about&act=wall_send_comm', {rec_id: rec_id, wall_text: wall_text, public_id: public_id}, function(data){
				$('#fast_form_'+rec_id+', #fast_comm_link_'+rec_id).remove();
				$('#wall_fast_block_'+rec_id).html(data);
				var pattern = new RegExp(/news/i);
				if(pattern.test(location.href)) $('#fast_text_'+rec_id+', #fast_inpt_'+rec_id).css('width', '688px');
	...
	wall_delet: function(rec_id){
		...
		myhtml.title_close(rec_id);
		updateNum('#rec_num');
		$.post('/index.php?go=about&act=wall_del', {rec_id: rec_id});
	},
	wall_page: function(){
		var page_cnt = $('#page_cnt').val();
		var public_id = $('#public_id').val();
		...
		if($('#load_wall_all_records').text() == 'к предыдущим записям' && $('#rec_num').text() > 10){
			textLoad('load_wall_all_records');
			$.post('/index.php?go=about&aboutid='+public_id, {page_cnt: page_cnt}, function(data){
				$('#public_wall_records').append(data);
				$('#page_cnt').val((parseInt($('#page_cnt').val())+1));
				if($('.wallrecord').size() == $('#rec_num').text()){
					$('#wall_all_records').hide();
				} else {
					$('#wall_all_records').attr('onClick', 'about.wall_page(\''+public_id+'\')');
					$('#load_wall_all_records').html('к предыдущим записям');
				}
			});
		}
	...
	wall_attach_addphoto: function(id, page_num, public_id){
		...
		Box.Page('/index.php?go=about&act=photos', 'aboutid='+public_id+page, 'c_all_photos_'+page_num, 625, 'Прикрепление фотографии', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	wall_attach_insert: function(type, data, action_url){
		if(!$('#wall_text').val())
			wall.form_open();
	...
	wall_photo_view: function(rec_id, public_id, src, pos){
		...
		if(size == 1){
			...
			var topTxt = 'Фотография <span id="pTekPost">'+pos+'</span> из '+size;
			var next = 'about.wall_photo_view_next('+rec_id+'); return false';
		}
		
		var content = '<div id="photo_view" class="photo_view" onClick="about.wall_photo_view_setEvent(event)">'+
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg cursor_pointer" onClick="'+next+'" style="min-height:400px">'+
  '<div class="photo_com_title" style="padding-top:0px;">'+topTxt+'<div><a href="/" onClick="Photo.Close(\'\'); return false">Закрыть</a></div></div>'+
  '<div class="photo_img_box"><img src="'+photo+'" id=\"photo_view_src\" style="margin-bottom:7px" /></div><div class="line_height">'+
...
'</div>';
	...
	wall_video_add_select: function(){
		...
		if(video_attach_lnk != 0){
			$('#box_loading').show();
			ge('box_butt_create').disabled = true;
			$.post('/index.php?go=about&act=select_video_info', {video_id: data[1]}, function(row){
				if(row == 1){
					addAllErr('Неверный адрес видеозаписи');
					$('#box_loading').hide();
					ge('box_butt_create').disabled = false;
				} else {
					about.wall_attach_insert('video', '/uploads/videos/'+data[0]+'/'+row, row+'|'+data[1]+'|'+data[0]);
					$('#video_attach_lnk').val('');
				}
			});
		} else
	...
	wall_add_like: function(rec_id, user_id, type){
		...
		$('#wall_like_cnt'+rec_id).html(wall_like_cnt).css('color', '#2f5879');
		$('#wall_active_ic'+rec_id).addClass('public_wall_like_yes');
		$('#wall_like_link'+rec_id).attr('onClick', 'about.wall_remove_like('+rec_id+', '+user_id+', \''+type+'\')');
		$('#like_user'+user_id+'_'+rec_id).show();
		updateNum('#like_text_num'+rec_id, 1);
		...
		if(type == 'uPages')
			...
			$.post('/index.php?go=about&act=wall_like_yes', {rec_id: rec_id});
	},
	wall_remove_like: function(rec_id, user_id, type){
		var wall_like_cnt = parseInt($('#wall_like_cnt'+rec_id).text())-1;
		if(wall_like_cnt <= 0){
		...
		$('#wall_like_cnt'+rec_id).html(wall_like_cnt).css('color', '#95adc0');
		$('#wall_active_ic'+rec_id).removeClass('public_wall_like_yes');
		$('#wall_like_link'+rec_id).attr('onClick', 'about.wall_add_like('+rec_id+', '+user_id+', \''+type+'\')');
		$('#Xlike_user'+user_id+'_'+rec_id).hide();
		$('#like_user'+user_id+'_'+rec_id).hide();
		updateNum('#like_text_num'+rec_id);
		...
		if(type == 'uPages')
			...
			$.post('/index.php?go=about&act=wall_like_remove', {rec_id: rec_id});
	},
	wall_like_users_five: function(rec_id, type){		
		$('.public_likes_user_block').hide();
		if(!ge('like_cache_block'+rec_id) && $('#wall_like_cnt'+rec_id).text() && $('#update_like'+rec_id).val() == 0){
			if(type == 'uPages'){
				...
				$.post('/index.php?go=about&act=wall_like_users_five', {rec_id: rec_id}, function(data){
					$('#likes_users'+rec_id).html(data+'<span id="like_cache_block'+rec_id+'"></span>');
					$('#public_likes_user_block'+rec_id).show();
				});
			}
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=about&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	vPhoto: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=abouts&act=vphoto', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var likephoto = {
wall_add_like: function(photo_id, user_id){
		if($('#wall_like_cnt'+photo_id).text()) var wall_like_cnt = parseInt($('#wall_like_cnt'+photo_id).text())+1;
		else {
			$('#public_likes_user_block'+photo_id).show();
			$('#update_like'+photo_id).val('1');
			var wall_like_cnt = 1;
		}
		
		$('#wall_like_cnt'+photo_id).html(wall_like_cnt).css('color', '#2f5879');
		$('#wall_active_ic'+photo_id).addClass('public_wall_like_yes');
		$('#wall_like_link'+photo_id).attr('onClick', 'likephoto.wall_remove_like('+photo_id+', '+user_id+')');
		$('#like_user'+user_id+'_'+photo_id).show();
		updateNum('#like_text_num'+photo_id, 1);
		
		$.post('/index.php?go=photo&act=plike_yes', {photo_id: photo_id});
	},
	wall_remove_like: function(photo_id, user_id){
		var wall_like_cnt = parseInt($('#wall_like_cnt'+photo_id).text())-1;
		if(wall_like_cnt <= 0){
			var wall_like_cnt = '';
			$('#public_likes_user_block'+photo_id).hide();
		}
		
		$('#wall_like_cnt'+photo_id).html(wall_like_cnt).css('color', '#95adc0');
		$('#wall_active_ic'+photo_id).removeClass('public_wall_like_yes');
		$('#wall_like_link'+photo_id).attr('onClick', 'likephoto.wall_add_like('+photo_id+', '+user_id+')');
		$('#Xlike_user'+user_id+'_'+photo_id).hide();
		$('#like_user'+user_id+'_'+photo_id).hide();
		updateNum('#like_text_num'+photo_id);

		$.post('/index.php?go=photo&act=plike_no', {photo_id: photo_id});
	},
	wall_like_users_five: function(photo_id){
		$('.public_likes_user_block').hide();
		if(!ge('like_cache_block'+photo_id) && $('#wall_like_cnt'+photo_id).text() && $('#update_like'+photo_id).val() == 0){
			$.post('/index.php?go=photo&act=pliked_users', {photo_id: photo_id}, function(data){
				$('#likes_users'+photo_id).html(data+'<span id="like_cache_block'+photo_id+'"></span>');
				$('#public_likes_user_block'+photo_id).show();
			});
		} else
			if($('#wall_like_cnt'+photo_id).text()) $('#public_likes_user_block'+photo_id).show();
	},
	wall_like_users_five_hide: function(){$('.public_likes_user_block').hide();},
	pall_liked_users: function(photo_id, page_num, liked_num){
		if(page_num)
			page = '&page='+page_num;
		else {
			page = '';
		...
		Box.Page('/index.php?go=photo&act=pall_liked_users', 'photo_id='+photo_id+'&liked_num='+liked_num+page, 'all_liked_users_'+photo_id+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
