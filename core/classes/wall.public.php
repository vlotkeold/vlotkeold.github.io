			...
			if($row_wall['attach']){
				...
				foreach($attach_arr as $attach_file){
					...
					if($attach_type[0] == 'photo' AND file_exists(ROOT_DIR."/uploads/groups/{$globParId}/photos/c_{$attach_type[1]}")){
						if($cnt_attach < 2)
							$attach_result .= "<div class=\"profile_wall_attach_photo cursor_pointer page_num{$row_wall['id']}\" onClick=\"groups.wall_photo_view('{$row_wall['id']}', '{$globParId}', '{$attach_type[1]}', '{$cnt_attach}')\"><img id=\"photo_wall_{$row_wall['id']}_{$cnt_attach}\" src=\"/uploads/groups/{$globParId}/photos/{$attach_type[1]}\" style=\"width:304px;\" align=\"left\" /></div>";
						else
...
HTML;
			...
			if($idu['user_id']) $idu = $row_wall['uid'];
			else $idu = $row_wall['uid'];
			...
			if($row_wall['comments'] OR $public_admin){
				if($row_wall['fasts_num']){
					...
					$sql_comments = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.id, public_id, text, add_date, type, for_groups, rec_answer, tb2.user_photo, user_search_pref FROM `".PREFIX."_communities_wall` tb1, `".PREFIX."_users` tb2 WHERE tb1.public_id = tb2.user_id AND tb1.fast_comm_id = '{$row_wall['id']}' ORDER by `add_date` ASC LIMIT {$comments_limit}, 3", 1);
					...
					foreach($sql_comments as $row_comments){
						...
						if($row_comments['for_groups']) {
							$row_public = $db->super_query("SELECT title, photo FROM `".PREFIX."_communities` WHERE id = '{$pid}'");
							$tpl->set('{name}', $row_public['title']);
							if($row_public['photo'])
								$tpl->set('{ava}', $config['home_url'].'uploads/groups/'.$pid.'/50_'.$row_public['photo']);
							else
								$tpl->set('{ava}', '{theme}/images/no_ava_50.png');
