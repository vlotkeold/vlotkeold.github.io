...
$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.*, tb2.user_search_pref, user_last_visit FROM `".PREFIX."_log` tb1, `".PREFIX."_users` tb2 WHERE tb1.uid = tb2.user_id {$where_sql} ORDER by `uid` DESC LIMIT {$limit_page}, {$gcount}", 1);
