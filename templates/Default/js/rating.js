...
if(add != 0){
  ...
  $.post('/index.php?go=rating&act=add', {for_user_id: u, num: add, comment: c}, function(d){
    ...
    if(trate > 1000){
      ...
      $('.profile_rate_100_r
ight').addClass('profile_rate_500_right');
...
if(add && new_rate >= 0){
  ...
  $('#num').text('РЅРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ');
  $('#rt').hide();
} else {
  $('#rt').show();
...
$.post('/index.php?go=rating&act=view', function(d){
  ...
  page: function(){
    if($('#load_rate_prev_ubut').text() == 'РџРѕРєР°Р·Р°С‚СЊ РїСЂРµРґС‹РґСѓС‰РёРµ РїРѕРІС‹С€РµРЅРёСЏ'){
  textLoad('load_rate_prev_ubut');
  $.post('/index.php?go=rating&act=view', {page_cnt: page_cnt_rate}, function(d){
page_cnt_rate++;
$('#rating_users').append(d);
$('#load_rate_prev_ubut').text('РџРѕРєР°Р·Р°С‚С
Њ РїСЂРµРґС‹РґСѓС‰РёРµ РїРѕРІС‹С€РµРЅРёСЏ');
