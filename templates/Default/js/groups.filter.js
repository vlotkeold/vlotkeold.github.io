jQuery.uiTableFilter = function (jq, phrase, column, ifHidden) {
    ...
    if ((words.size > 1) && (phrase.substr(0, phrase_length - 1) === this.last_phrase)) {
        ...
        var elems = jq.find("tbody > tr > td > div:visible")
		
    } else {
        new_hidden = true;
        var elems = jq.find("tbody > tr > td > div")
    }
...
$(function () {
    var theTable = $('table#main_panel');
    theTable.find("tbody > tr > td").find("div:eq(1)").mousedown(function () {
        $(this).prev().find(":checkbox").click();
    });
    $("#filter").keyup(function () {
        $.uiTableFilter(theTable, this.value);
		var length = theTable.find("tbody > tr > td > div:visible").length;
		if(length>0) $('#no_search').hide();
		else $('#no_search').show();
    })
});