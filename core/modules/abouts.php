	...
	switch($act){
		...
		case "send":
			...
			if(isset($title) AND !empty($title)){
				...
				$db->query("INSERT INTO `".PREFIX."_about` SET title = 'О сайте', type = 1, traf = 0, ulist = '|{$user_id}|', date = NOW(), admin = 'u{$user_id}|', real_admin = '{$user_id}', time={$server_time}, comments = 1, privacy = 'p_contact|1||p_audio|1||'");
				$cid = $db->insert_id();
		...
		case "allfeedbacklist":
			...
			if(stripos($owner['admin'], "u{$user_id}|") !== false)
				echo "<style>#box_bottom_left_text{padding-top:6px;float:left}</style><script>$('#box_bottom_left_text').html('<a href=\"/\" onClick=\"about.addcontact({$id}); return false\">Добавить контакт</a>');</script>";
