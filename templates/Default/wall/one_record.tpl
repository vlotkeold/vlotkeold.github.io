...
[yesedit][yesedittime]<div class="editwall" onMouseOver="myhtml.title('{rec-id}', '<b>Редактировать</b>', 'wall_edit_')" onClick="wall.edit_form('{rec-id}'); return false" id="wall_edit_{rec-id}"></div>[/yesedittime][/yesedit][/owner]
 ...
 [owner-record]<div class="wall_delete" onMouseOver="myhtml.title('{rec-id}', '<b>Отметить как спам</b>', 'wall_spam_')" onClick="Report.WallSend('wall', '{rec-id}'); return false"id="wall_spam_{rec-id}"></div>[/owner-record]
...
[comment-form]<div class="wall_fast_opened_form" id="fast_form" style="width:550px;">
 <input type="text" class="wall_inpst fast_form_width wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); return false" style="width:540px;margin:0px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
